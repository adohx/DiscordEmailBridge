...
最简单的情况下，`TARGET_EMAILS` 只填一个地址，且和 `ALLOWED_EMAIL_SENDER` 是同一个人的邮箱。`TARGET_EMAILS` 支持逗号分隔的多个收件人（例如 `TARGET_EMAILS=alice@example.com,bob@example.com`），每个收件人都会收到 Discord 消息、编辑和删除通知的邮件。`ALLOWED_EMAIL_SENDER` 目前仍然只支持一个回复邮箱，其他地址发来的回复会被忽略（见 `mail_reader.poll_mailbox`）。
