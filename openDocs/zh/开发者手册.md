...
3. 在 Bot 页面：
   ...
   **Public Bot 保持默认开启就行，不用关**。理论上关掉能防止别人把 Bot 加到自己的 Server，但实测这一步会引出额外的麻烦（比如已经邀请过的情况下改这个开关）。实际的访问控制已经在代码层做了，不依赖这个开关：`discord_client.py` 的 `_is_bridged_channel` 只处理 `channel.id == DISCORD_CHANNEL_ID` 的消息（Channel ID 是 Discord 全局唯一的雪花 ID，其他 Server 不可能凑巧撞上），配了 `DISCORD_GUILD_ID` 的话还会先比对 Server ID。也就是说，就算这个 Bot 被别人加进了别的 Server，它也只会读写咱们指定的这一个频道，别的地方一律忽略。
...
5. 用最小权限把 Bot 邀请到目标 Server 和目标 Channel——详细图文步骤见《用户手册》第 1 节（生成邀请链接、选择 Server、勾选 `Send Messages` + `Read Message History` + `Attach Files`、在目标频道单独开 `View Channel`），这里不重复贴图。`Attach Files` 是转发邮件附件（图片、PDF、DOC、DOCX）必需的权限——没有这个权限，转发会降级成纯文字消息 + "上传失败"提示（`discord.errors.Forbidden: 403 Forbidden (error code: 50013): Missing Permissions`）。
...
7. （建议做）记下目标 Server 的 ID（右键 Server 图标 → 复制服务器 ID），后面填进 `DISCORD_GUILD_ID`。这个字段本身是可选的，不填也不影响功能（`DISCORD_CHANNEL_ID` 的过滤已经够用），但既然 Public Bot 开关没关，多这一层 Server ID 校验更保险。
...
|        `TARGET_EMAILS`        |  是  | 接收 Discord 消息的目标邮箱，逗号分隔，可以只填一个 |
|   `ALLOWED_EMAIL_SENDERS`     |  是  | 允许回复并转发回 Discord 的邮箱地址，逗号分隔，可以只填一个 |
| `ALLOWED_EMAIL_SENDER_NAMES`  |  是  | 每个发件人在 Discord 里显示的名字，逗号分隔，**按位置和 `ALLOWED_EMAIL_SENDERS` 一一对应**（第 N 个名字对应第 N 个邮箱）；不是从邮件里解析的，需要手动配置 |
...
最简单的情况下，`TARGET_EMAILS` 和 `ALLOWED_EMAIL_SENDERS` 都只填一个地址，且是同一个人的邮箱。两者都支持逗号分隔的多个值（例如 `TARGET_EMAILS=alice@example.com,bob@example.com`）——`TARGET_EMAILS` 里每个收件人都会收到 Discord 消息、编辑和删除通知的邮件；`ALLOWED_EMAIL_SENDERS` 里每个地址发来的回复都会被接受，其他地址会被忽略（见 `mail_reader.poll_mailbox`）。`ALLOWED_EMAIL_SENDER_NAMES` 必须和 `ALLOWED_EMAIL_SENDERS` 按位置对齐——两边逗号分隔的个数对不上，程序启动时就会直接报错，而不是把回复默默地记到错的名字上。
...
`uv sync` 会顺带装好 `dev` 依赖组（`pytest`、`pytest-asyncio`）。在 `discord-email-bridge/` 目录下跑全部测试：

```bash
uv run pytest
```

测试不会连真实的 Discord/SMTP/IMAP 服务器——Discord 和网络调用都是 mock 出来的，`state.py` 相关测试用的是临时文件。如果你的 shell 里 source 过 ROS 之类会注入自己 `PYTHONPATH` 的东西，可能会把不相关的 pytest 插件泄漏进这个 venv，导致启动阶段报一个无关的 `ModuleNotFoundError`；遇到这种情况改用 `PYTHONPATH= uv run pytest` 即可。

长期运行建议用 systemd（Ubuntu 场景）。仓库里已经有现成的 `discord-email-bridge.service` 模板和 `deploy.sh` 部署脚本，脚本会创建专用的 `discordbridge` 系统用户、把项目拷贝到 `/opt/discord-email-bridge`，再安装并启用 systemd 服务：

```bash
sudo ./deploy.sh
# 然后编辑 /opt/discord-email-bridge/.env 填入真实凭证
sudo systemctl start discord-email-bridge
sudo systemctl status discord-email-bridge
journalctl -u discord-email-bridge -f
```

如果想用不同的用户名/路径，运行脚本前设置 `APP_USER` / `APP_DIR` 环境变量覆盖默认值即可。如果想手动部署，改一下 `discord-email-bridge.service` 里的 `WorkingDirectory` / `ExecStart` / `User`，自己 `systemctl enable --now` 也可以。
...
| `discord_client.py`| Discord 端。`BridgeClient`（`discord.Client` 子类）监听 `on_message` / `on_raw_message_edit` / `on_raw_message_delete` / `on_raw_reaction_add`，做 guild/channel/bot 过滤后回调给 `main.py`；`deliver_email_to_channel` 负责把邮件内容投递进频道（优先做成真正的 Discord 回复），并处理长度截断、mention 清理。Reaction 不需要额外申请 Discord 权限——`guild_reactions` 是非特权 intent，`Intents.default()` 已经默认包含。 |
|  `mail_reader.py`  | 邮件读取端（IMAP）。`poll_mailbox` 连接邮箱、拉取未读邮件，过滤发件人白名单（`allowed_email_senders`）和去重，提取纯文本正文并剥离引用历史，解析 `In-Reply-To`/`References`/`X-Discord-Bridge-ID` 判断回复关系，通过回调交给 `main.py`。 |
|  `mail_sender.py`  | 邮件发送端（SMTP）。把 Discord 消息包装成邮件（新消息 / `[Updated]` / `[Deleted]` / `[Reaction]`），生成 `Message-ID` 及线程头，通过 SMTP 发出。                          |
...
刻意做得很窄：只有 `mapping["origin"] == "email"` 的消息（也就是邮件用户自己发的消息，在 `handle_incoming_email` 调 `add_mapping` 时设置）收到 reaction 才会发通知。频道里其他 Discord 原生消息的 reaction，以及取消 reaction，都不转发——不然活跃频道里此起彼伏的 reaction 会把邮件用户的收件箱刷爆，跟这个桥接工具"安静收件箱"的初衷背道而驰。发完通知不写状态（跟编辑/删除不一样），因为 reaction 通知不会改变映射本身持久化的任何字段。

`state.json` 是这一整套流程里唯一的持久化状态，保存 Discord 消息 ID 与 Email Message-ID 之间的双向映射。"邮件回复能变成 Discord 真实回复""编辑/删除/reaction 通知能找到原始邮件"，都建立在这份映射之上——具体判定规则见 3.1。
...
|    `mail_reader.py`    | · 能够读取邮件（IMAP轮询）<br/>· 仅信任`ALLOWED_EMAIL_SENDERS`里的邮箱发来的邮件，其余邮件直接忽略<br/>· 邮件正文优先取纯文本，去除引用历史（"On ... wrote:"等） |     已完成     |         手工测试完成          |   0.1    |
...
HTML 邮件支持（0.4）：`mail_reader._extract_plain_text` 目前遇到只有 HTML 正文（没有 `text/plain` 部分）的邮件会直接跳过并记日志，不转发到 Discord（属于 MVP 阶段明确排除的范围）。要支持需要把 HTML 转成可读纯文本（比如用简单的标签剥离或者 `html2text` 之类的库），再走现有的去引用历史逻辑，属于 `mail_reader.py` 内的局部改动，不涉及状态模型或消息映射规则的变化。
